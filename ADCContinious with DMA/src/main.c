...
static const uint32_t ADCDataBuffer[ADCBufferSize];		// Array to store data from ADC
...
void initDMA(void)
{
  ...
  DMASetup.DMA_BufferSize = 2;					// How many transfers from register to memory
  ...
  DMASetup.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;			// 16 bits
  DMASetup.DMA_Mode = DMA_Mode_Circular;						// When all convertions are done, start again
...
int main(void)
{
	...
	while(1)
	{
		...
		Ain = ADC_GetConversionValue(ADC1);			// Fetch data by software to compare against DMA ADC_DataBuffer
		Delay(1000);
	}
